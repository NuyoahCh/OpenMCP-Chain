...
> 智能体执行是异步的，`thought`/`reply` 等详细结果可通过轮询 `GET /api/v1/tasks/{task_id}`（或兼容的 `GET /api/v1/tasks?id=...`）获取。
...
- `tasks`：匹配过滤条件的任务数组，字段与任务实体保持一致。若需要查询单条记录，可使用 `GET /api/v1/tasks/{task_id}`（兼容 `?id=<task_id>` 查询参数）。
- `total`：符合当前过滤条件的任务总数。
